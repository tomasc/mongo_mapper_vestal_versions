...
class VersionsTest < Test::Unit::TestCase
  context 'A collection of associated versions' do
    ...
    should 'be fetchable by version number' do
      @times.keys.each do |number|
        assert_kind_of MongoMapper::Plugins::VestalVersions::Version, @user.versions.at(number)
    ...
    should 'be fetchable by tag' do
      @times.keys.map{|n| [n, n.to_s] }.each do |number, tag|
        assert_kind_of MongoMapper::Plugins::VestalVersions::Version, @user.versions.at(tag)
    ...
    should "be fetchable by the exact time of a version's creation" do
      @times.each do |number, time|
        assert_kind_of MongoMapper::Plugins::VestalVersions::Version, @user.versions.at(time)
    ...
    should "be fetchable by any time after the model's creation" do
      @times.each do |number, time|
        assert_kind_of MongoMapper::Plugins::VestalVersions::Version, @user.versions.at(time + 30.minutes)
    ...
    should 'be fetchable by an association extension method' do
      assert_kind_of MongoMapper::Plugins::VestalVersions::Version, @user.versions.at(:first)
      assert_kind_of MongoMapper::Plugins::VestalVersions::Version, @user.versions.at(:last)
    ...
    should 'be fetchable by a version object' do
      @times.keys.each do |number|
        ...
        assert_kind_of MongoMapper::Plugins::VestalVersions::Version, version
        assert_kind_of MongoMapper::Plugins::VestalVersions::Version, @user.versions.at(version)
