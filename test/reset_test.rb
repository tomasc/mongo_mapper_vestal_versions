  ...
  context 'Resetting a model' do
    ...
    context 'with the :dependent option as :delete_all' do
      ...
      should 'delete all versions after the target version' do
        @versions.reverse.each do |version|
          ...
          later_versions.each do |later_version|
            assert_equal nil, @user.versions.detect{|v|v==later_version}
          end
        end
      end
      ...
      should 'not destroy all versions after the target version' do
        @versions.reverse.each do |version|
          earlier_versions = @user.versions.before(version)
          @user.reset_to!(version)
          earlier_versions.each do |earlier_version|
            assert @user.versions.include?(earlier_version)
          end
        end
      end
    end
    
    # .destroy is NOT APPLICABLE on Embedded Documents
    ...
    # end
    # 
    # .nullify is NOT APPLICABLE on Embedded Documents
    # 
